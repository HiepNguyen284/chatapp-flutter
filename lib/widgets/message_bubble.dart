...
class MessageBubble extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final senderLabel = (senderName ?? '').trim().isEmpty
        ? (message.sender ?? 'User')
        : senderName!.trim();
    final showAvatarSlot =
        !isMine && (showSenderAvatar || reserveSenderAvatarSpace);
    final showReadReceipt =
        isMine && (seenByAvatars.isNotEmpty || deliveryStatus != null);
              ...
              child: Column(
                ...
                children: [
                  if (!isMine && showSenderName)
                    Padding(
                      padding:
                          const EdgeInsets.only(left: 14, right: 14, bottom: 1),
                      child: Text(
                        senderLabel,
